...
#======================== Starting up and Shutting Down================    
    ...
    def read(self, str):
        ...
        if self.in_sim_mode:
            if self.ser:
                self.ser.read( str )
            message = self.sim_sock.recv( str )
        else:
            message = self.ser.read( str )
        return unicode(message, encoding='Latin-1');
...
#========================= Moving Around ================================================    
    ...
    def driveDirect( self, leftMmSec=0, rightMmSec=0 ):
        ...
        """
        if leftMmSec < -500: leftMmSec = -500
        if leftMmSec > 500:  leftMmSec = 500
        if rightMmSec < -500: rightMmSec = -500
        if rightMmSec > 500: rightMmSec = 500
        
        # convert to mm/sec, ensure we have integers
        leftHighVal, leftLowVal = toTwosComplement2Bytes( int(leftMmSec) )
        rightHighVal, rightLowVal = toTwosComplement2Bytes( int(rightMmSec) )
...
#======================= CARGO BAY OUTPUTS ==========================
    ...
    def setDigitalOutputs(self, digOut2, digOut1, digOut0):
        ...
        data_byte = int("00000"+unicode(digOut2)+unicode(digOut1)+unicode(digOut0),2)
    ...
    def setLowSideDrivers(self, driver2, driver1, driver0):
        ...
        data_byte = int("00000"+unicode(driver2)+unicode(driver1)+unicode(driver0),2)
    ...
    def setPWMLowSideDrivers(self, dutyCycle2, dutyCycle1, dutyCycle0):
        ...
        self.__sendmsg(COMMANDS["PWM_LOW_SIDE_DRIVERS"],
                       chr(dutyCycle2)+unichr(dutyCycle1)+unichr(dutyCycle0))
