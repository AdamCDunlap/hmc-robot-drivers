#!/usr/bin/python
##########################################################
## Localization and turn by preemptive algorithm
    ...
    if D.START_FOUND:    
        ...
        if hallway_type != "ST": # if we have met an intersection
            ...
            if len(D.possibleNodes) == 1:
                ...
                # Eliminates un-matched nodes from the list
                estimateNextNodes(turn)
...
def preemptive():
    ...
    # generate all the path for every possible node
    for i in range(len(D.possibleNodes)):
        D.node_states[D.start_node] = EMPTY
        D.start_node = int(D.possibleNodes[i][0].name)
        D.node_states[D.start_node] = START
        D.curr_node = D.start_node
        # adjust the direction
        if D.possibleNodes[i][2] == NORTH:
            D.direction = SOUTH
        elif D.possibleNodes[i][2] == SOUTH:
            D.direction = NORTH
        elif D.possibleNodes[i][2] == EAST:
            D.direction = WEST
        elif D.possibleNodes[i][2] == WEST:
            D.direction = EAST
        ...
        firstCommand.append(D.commands[0])
        
    # emtpy start node
    D.node_states[D.start_node] = EMPTY
    
    # find the most popular one
    numL = [len(filter(lambda x: x[0] == LEFT, firstCommand)),LEFT]
    numR = [len(filter(lambda x: x[0] == RIGHT, firstCommand)),RIGHT]
    numS = [len(filter(lambda x: x[0] == STRAIGHT, firstCommand)),STRAIGHT]
    numT = [len(filter(lambda x: x[0] == TURNAROUND, firstCommand)),TURNAROUND]
    popNumList = [numL[0],numR[0],numS[0],numT[0]]
    popList = [numL,numR,numS,numT]
    print popNumList
    print popList
    return popList[popNumList.index(max(popNumList))][1]
...
def estimateNextNodes(turn):
    ...
    for curr_node in D.possibleNodes:
        if turn == RIGHT:
            ...
            nextNeighbor, nextNode, dir_nextNode = getBackNeighbors(curr_node)      # get neighbor list of the "next" node
...
def onMouse(event, x, y, flags, param):
    ...
    if event==cv.CV_EVENT_LBUTTONDOWN:
        ...
        print "Start node is reset as " + str(D.start_node)
        D.curr_node  = D.start_node
