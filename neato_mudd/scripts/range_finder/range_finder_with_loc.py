...
##    Authors: Cyrus Huang, Zakkai Davidson, Jerry Hsiung
##    Date: JUNE&JULY 2013
##    How it works: When getting to a corner, the neato will broadcast the corner
##    type to the map, and the map tells it how to turn
...
def findHoughLines():
    ...
    # check all the hough lines and get the information we need
    for line in lines:
        cv.Line(D.color_dst, line[0], line[1], cv.CV_RGB(0, 255, 0), 1, 8)

        # storing the lines and their information
        ...
        # find the midpoint, and the angle to center
...
def wallFollow():
    ...
    try:
        ...
        # broadcast and receive the command
        broadcast()
        ...
        # hallway following based on the most vertical line
        else:
            theta = 0
...
def turn(direction):
    """turn the neato based on the direction command"""
    global D
    # reset the next_dir
    D.next_dir = ""
    if direction == "L":
...
def broadcast():
    ...
    # calculate the how many walls are blocked
    ...
    # When the counter reaches some number, broadcast it
