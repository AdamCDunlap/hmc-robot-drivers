#!/usr/bin/python
##########################################################
## Localization and turn by calculating the entropy scores 
...
D.possibleNodes = []
D.drawPossNodes = []
D.entropyScores = [0,0,0,0] # left, right, straight, turnaround
D.entropyLists = [0,0,0,0] # left, right, straight, turnaround
...
def generateList():
    ...
    for node_nums in NODE_NUMS:
        ...
        # Check if the robot meets a corner
...
def command_callback(data):
    ...
    if D.START_FOUND:    
        ...
        if hallway_type != "ST": # if we have met an intersection
            ...
            # Check if there is only one node left             
            if len(D.possibleNodes) == 1:
                print "The unique node is " + str(D.possibleNodes)
                ...
                # Decide the next move by checking the entropy scores of every possible turn
                for turn in TURN:
                    try:
                        D.entropyScores[TURN.index(turn)],D.entropyLists[TURN.index(turn)] = scoreNextNodes(turn)
                    except:
                        D.entropyScores[TURN.index(turn)] = 100000
                        pass # if some turn is not possible, pass and give a high score
                    print turn, D.entropyScores[TURN.index(turn)]

                # Find the entropy scores and decide which direction to turn
                minIndex = D.entropyScores.index(min(D.entropyScores))
                D.possibleNodes = D.entropyLists[minIndex]
                message = NavCommand()
                message.command = TURN[minIndex]
                print "The robot is going to turn", TURN[minIndex]
                D.commandPub.publish(message)

    D.prev_type = hallway_type


def scoreNextNodes(turn):
    """ Given a turn, this function evaluates the entropy score of all the next sets of nodes"""
    # get neighbors 
    ...
    # initialize the entropy score list
    scoreList = []
    possibleList = []
    
    # check all the possible neighbors and put them in a list according to turning direction
    for curr_node in D.possibleNodes:
        ...
        # append to final estimation list
        nextNode_type = nextNode.type
        scoreList.append([nextNode, nextNode_type, dir_nextNode])
        possibleList.append([nextNode, corner_type_nextNode, dir_nextNode])

    # Scoring (Sums up expected values of each corner type)
    numL = len(filter(lambda x: x[1] == ELBOW, scoreList))
    numT = len(filter(lambda x: x[1] == T_INT, scoreList))
    numP = len(filter(lambda x: x[1] == CROSS, scoreList))
    numD = len(filter(lambda x: x[1] == DEAD_END, scoreList))

    # Weighted average score
    score = 1.0*(numL**2 + numT**2 + numP**2 + numD**2)/len(scoreList)
    
    return score, possibleList
