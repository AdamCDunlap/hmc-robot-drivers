...
class Create:
    ...
    def start(self):
        ...
        self.__sendNow(128,128,132)
        self.playFullSong([60,62,64,65,64,60,67],[8]*7)
        #sleep(1)
    ...
    def restart(self):
        """restart the iCreate after docking"""
...
#       self.stop()
    ...
    def playFullSong(self,notes,durations):
        """ Play an arbitrarily long song """
        """ added summer 2012 """
        
        """ The irobot can only play song 16 notes at a time so
            we split the song into chunks """
        totalNotes = min(len(notes),len(durations))
        for x in xrange(0, totalNotes, 16):
            self.storeSong(0,notes[x:x+16],durations[x:x+16])
            self.send(141,0)
            # wait for the song to finish playing
            sleep( sum(durations[x:x+16])/64.0)

    def storeSong(self, num, notes, durations):
        ...
        """ Edited summer 2012 """
        numNotes = min(len(notes),len(durations),16)
        notes = notes[:numNotes]
        durations = durations[:numNotes]
        songMsg = []

        for note,duration in zip(notes,durations):
            songMsg += [note,duration]

        self.send(140,num,numNotes,*songMsg)
