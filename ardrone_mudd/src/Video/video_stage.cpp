...
C_RESULT output_gtk_stage_open( void *cfg, vp_api_io_data_t *in, vp_api_io_data_t *out)
{
    ...
    nv = new ros::NodeHandle;
    it = new image_transport::ImageTransport( *nv );
    vpub = new image_transport::Publisher( it ->advertise("droneImage",1) );
    return (SUCCESS);
}
...
C_RESULT output_gtk_stage_transform(  vlib_stage_decoding_config_t *cfg, vp_api_io_data_t *in, vp_api_io_data_t *out)
{
  ...
  IplImage *img = ipl_image_from_data(pixbuf_data);
...
DEFINE_THREAD_ROUTINE(video_stage, data)
{
  ...
  picture.y_buf   = (uint8_t *)vp_os_malloc( QVGA_WIDTH * QVGA_HEIGHT     );
  picture.cr_buf  = (uint8_t *)vp_os_malloc( QVGA_WIDTH * QVGA_HEIGHT / 4 );
  picture.cb_buf  = (uint8_t *)vp_os_malloc( QVGA_WIDTH * QVGA_HEIGHT / 4 );
